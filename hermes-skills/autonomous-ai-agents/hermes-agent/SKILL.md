...
- **Primary:** `minimax-m2.5-free` via OpenCode Zen (free tier, no API key needed)
- **Local backup:** `qwen2.5-coder:32b` via Ollama at `http://localhost:11434/v1`

**All free models on OpenCode Zen (as of May 2026):**

| Model ID | Status |
|---|---|
| `minimax-m2.5-free` | ✅ Default — used by all cron jobs and main session |
| `deepseek-v4-flash-free` | Free (limited time, OpenCode collecting feedback) |
| `nemotron-3-super-free` | Free (limited time, OpenCode collecting feedback) |
| `big-pickle` | Free stealth model (limited time) |

**Paid models (pay-as-you-go, per 1M tokens — input/output):**
...
Auto-reload: when balance drops below $5, Zen auto-adds $20. Monthly spend limits available.

Full pricing: https://opencode.ai/docs/zen#pricing
...
| OpenCode Zen | API key | `OPENCODE_ZEN_API_KEY` |
| Ollama (local) | None | Requires Ollama installed and model loaded; base_url `http://localhost:11434/v1` |
| Custom endpoint | Config | `model.base_url` + `model.api_key` in config.yaml |

### Fallback Provider Chain (Local Backup)

You can configure a fallback chain so Hermes automatically switches to a backup provider if the primary fails or hits rate limits. A common pattern: **OpenCode/Minimax (primary, cloud) → Ollama (backup, local)**.
...
providers:
  ollama-local:
    ...
    default: qwen2.5-coder      # or any model you have loaded in Ollama
...
**Requirements:**
- Ollama must be running (`ollama run <model>` or `ollama serve`)
- Model must be loaded before the fallback triggers
- Verify Ollama is responding: `curl http://localhost:11434/v1/models`

**Test the fallback manually:**
```bash
hermes chat --provider ollama-local "test"
```

**Restart after config changes:** The gateway snapshots config at startup. Use `/restart` (gateway), `hermes gateway restart` (CLI), or:
```bash
pkill -f "hermes.*gateway" && hermes gateway &
```
The PID file (`~/.hermes/gateway.pid`) may be stale — if `pkill -F gateway.pid` says "Invalid pid in file", use `pkill -f hermes.*gateway` instead.
...
**One-shot with local Ollama model (via delegate_task):**
```
delegate_task(goal="Write a Python function...", toolsets=["terminal", "file"])
```

**One-shot with Codex CLI + Ollama:**
```bash
cd /tmp && git init -q && \
export PATH="/opt/homebrew/opt/ollama/bin:$PATH" && \
codex --oss --local-provider ollama --model qwen2.5-coder:32b exec 'prompt'
```
Requires: `pty=true`, git repo, Ollama 0.24+ running, PATH set.

**Known issue:** Cold start timeout on first call (~24 tok/s for 32B models). Works on second call.
To upgrade: `brew upgrade ollama`.
...
**Weekly System Update Pattern**

Create a weekly cron job that audits and updates all package managers on macOS:

```
/cron create "0 8 * * 0"  # Every Sunday 8 AM (user timezone +03:00)
```

Prompt pattern:
1. `brew outdated` — all outdated Homebrew packages
2. `npm outdated -g --depth=0` — global npm packages
3. `pip list --outdated` — pip packages
4. `gem outdated` — Ruby gems
5. Report summary (current vs latest)
6. Auto-run: `npm update -g`, `brew upgrade` (background with `notify_on_complete=true` since it upgrades 60+ packages), pip/gem upgrades as needed
7. Report final summary with errors

**Pitfall:** Run `brew upgrade` in background with `notify_on_complete=true` — it takes several minutes and upgrades many packages. Do NOT run it synchronously.
...
- **Bulk model migrations:** list jobs, then update each `job_id`
  individually. The cron tool has no one-shot "set all jobs to model X"
  operation. See `references/cron-model-migrations.md` for the exact
...
## Terminal & execute_code Workarounds

### Changing the default working directory

`TERMINAL_CWD` is bridged from `terminal.cwd` in `config.yaml`. The primary fix for path issues is a config change, not a workaround:

```bash
hermes config set terminal.cwd /path/to/dir   # or edit config.yaml directly
```

**Restart required** — the gateway snapshots this at startup. After changing, use `/restart` (gateway) or restart the CLI.

### When `terminal` tool fails with path errors

If `terminal` fails with errors like `cd: /home/node/.openclaw/workspace: No such file or directory`, the usual cause is an invalid `TERMINAL_CWD` pointing at a non-existent directory. In this case, use `execute_code` (sandboxed Python) to probe environment state instead:

```python
import os
for k, v in sorted(os.environ.items()):
    if any(x in k for x in ["HOME", "HERMES", "TERMINAL", "PYTHON", "VIRTUAL"]):
        print(f"{k}={v}")
```

This works because the sandbox doesn't inherit the broken `TERMINAL_CWD` from the container startup.

Also note: commands requiring approval for dotfile overwrites (e.g. `>> ~/.zshrc`) may get flagged by security scans. Prefer direct user instruction or confirm before redirecting to dotfiles.

### Output truncation: model.max_tokens vs tool_output.max_bytes

Two separate limits control how much output can be produced and delivered:

1. **`model.max_tokens`** (config: `config.yaml → model → max_tokens`) — max output tokens the LLM can generate in one response. Default is often too low (e.g. 1000). For tasks producing long content (news briefings, research reports), bump this to 8000–16000.

2. **`tool_output.max_bytes`** (config: `config.yaml → tool_output → max_bytes`) — max size of a tool's output string that can be delivered back (50KB default). If the agent's response text itself is large, this caps it too. Bump to 150000+ for content-heavy jobs.

**Symptom:** `RuntimeError: Response truncated due to output length limit` — the model ran out of output tokens AND the tool output limit was hit. Fix both: raise `model.max_tokens` AND `tool_output.max_bytes`, then restart the gateway (`/restart` in gateway mode, or kill and re-spawn in CLI).

**Working values for content-heavy cron jobs:**
```yaml
model:
  max_tokens: 16000    # was 1000, raised for news briefings
tool_output:
  max_bytes: 150000    # was 50000, raised for long outputs
```

**No restart = no effect.** Config changes are snapshot at gateway startup. The gateway PID file (`~/.hermes/gateway.pid`) can be stale — if `pkill -F gateway.pid` fails with "Invalid pid in file", use `pkill -f hermes.*gateway` or the `/restart` slash command from the gateway chat instead.

### Cron job pitfalls

- **`run` works on paused jobs** — no need to `resume` before `run`. Both can be triggered independently.
- **`pkill -F gateway.pid` may fail** if the PID file is stale/invalid (`Invalid pid in file`). Use `pkill -f hermes.*gateway` instead, or restart via `/restart` slash command in gateway.
- **`hermes gateway --daemon` does not exist.** To start gateway in background: `terminal(background=true, command="hermes gateway")`. Verify it's up with `hermes cron list` or `tail ~/.hermes/logs/gateway.log`.
- **Cron job output files** are stored at `~/.hermes/cron/output/<job_id>/<timestamp>.md` — useful for debugging failed runs.
