...
- **Worklog "started" format:** `YYYY-MM-DDThh:mm:ss.000-0700` (offset, no colon). Missing → 400; **malformed → HTTP 500 with an XML body `Error parsing time: <the bad value>`** (verified 2026-09-04) — the body quotes the string you sent, so read it rather than guessing at the payload.
...
- **EISHELP issues reject worklogs (verified 2026-09-10):** `POST /issue/EISHELP-*/worklog` → **400 `"you do not have the permission to associate a worklog to this issue." + "Worklog must not be null."`** — no Work-On-Issues permission in the EISHELP (helpdesk) project, regardless of issue state. Fulfilling an EISHELP access request is still billable: log the hour on the **linked delivery ticket** instead — `GET /issue/{EISHELP-KEY}?fields=issuelinks` names it (e.g. EISHELP-113018 → `Dependency` → GENESIS-441751), and reference the EISHELP key in the worklog comment.
...
## Helper — `~/.claude/scripts/jira-log-day.sh`
Posts a day's worklogs and then **re-reads** the day's rollup instead of trusting the 201s
(the daily and Friday tasks can double-log the same day). Aborts before writing anything if
the `GET /myself` pre-flight is not 200 — so an off-VPN 302 can never leave a partial day.
```bash
zsh -lic '~/.claude/scripts/jira-log-day.sh 2026-08-27 COEXT-107164:5:"observascope objstore secret" GENESIS-429534:3'
```
