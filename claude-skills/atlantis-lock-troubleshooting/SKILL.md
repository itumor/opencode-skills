...
- **Holder MR is open with an UNAPPLIED plan — including your own** → also normal, and the most common form: two open MRs touch the same dir, the second's autoplan dies at creation with `Plan Failed: This project is currently locked by an unapplied plan from pull !N`. Nothing is broken; Atlantis serializes per `<repo>/<dir>/<workspace>`, so stacking the follower onto the holder's branch does NOT help — the lock is per dir, not per branch. Two valid resolutions, and **the choice is a judgment call about merge order, not mechanics**:
  1. Let the holder apply+merge, then rebase the follower and comment `atlantis plan -p <project>` (rebase first if both touch the same file).
  2. Comment `atlantis unlock` on the **holder** MR — safe and reversible: it discards only the stored plan, the full plan text stays in that MR's notes, and one `atlantis plan -p <project>` regenerates it identically. Leave a note there saying why, so nobody thinks work was lost.
  Pick (2) when the follower should land first — e.g. a one-line fix that removes a diff the holder's own plan is carrying as noise. Live 2026-09-07/08, `credit-agricole/terraform` `upper/stage/services`: !142 (COEXT-109103 `rds.force_ssl` apply_method one-liner) was blocked by !141 (COEXT-108811 portal CloudFront), both mine. Unlocked !141 → !142 planned `No changes.` → !141's re-plan then reads `26 add / 0 change / 0 destroy` instead of carrying the phantom `1 change`. See [[atlantis-single-lock-cross-mr]], [[rds-parameter-apply-method-convergence]].
