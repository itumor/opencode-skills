...
### "Migrate the VMs to UAT" pushback — answer (hit live, COEXT-105281)
The CyberArk team may say the VMs are "in infra but we specified UAT, please migrate to the UAT environment." **Do NOT relocate them into the test/UAT VPC** — infra placement is the intended target, not a mistake. Talking points: (1) infra IS the per-account shared-services zone where *all* client EC2s live (GitLab/Jenkins/Nexus/Keycloak/build hosts); (2) the connectors serve every stage over the **transit gateway** — verify with `aws ec2 describe-route-tables` on the SIA subnet (expect `0.0.0.0/0 → tgw-...`, which routes to the test/UAT CIDR e.g. 10.34.88.0/23); (3) "serving UAT" = TGW reachability + onboarding the specific UAT hosts/DBs into CyberArk + opening the *target-side* SG port to the connector — NOT the connector's own VPC; (4) relocating breaks the shared model and forces a second pair at prod. Loop the EIS infra owner (Aurimas Kerpauskas) to align before any architecture change. Reachability is a route/SG verification step, never a VM migration.
