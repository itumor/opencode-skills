...
## Variant: onboard a non-SIA/PSM TARGET host into CyberArk (`CyberArkManaged` tag) (COEXT-105506, E2E 2026-06-18)
The operator perm set scopes StartSession to `Role∈{CyberArkSIA,CyberArkPSM}` (the connector VMs). To let the operator reach a **managed TARGET** (e.g. a build host, `Role=Build`) for onboarding, don't widen by `Role` — add a dedicated opt-in tag:
- **Part 1 (infra TF):** tag ONLY the target `CyberArkManaged=true`. In the eis-ec2 fleet ec2-map, **put the tag in `ec2_settings.<acronym>.tags` (type `any`), NOT the per-instance `var.ec2` entry** — `var.ec2` is `map(any)`; a nested `tags` object on just one entry fails plan with `all map elements must have the same type` (the other `{}`/`{custom_policy=true}` entries differ). `ec2_settings.<acronym>.tags` merges to the instance via ec2.tf, and if that acronym has one instance in the stage it still tags exactly that host. Plan must be in-place (`0 destroy`). Ref: credit-agricole/terraform!82.
- **Part 2 (IdC admin):** add ONE statement to the operator perm set, keyed on the tag so it generalises (no per-instance-id churn): `{"Sid":"SessionToManagedTargets","Effect":"Allow","Action":"ssm:StartSession","Resource":"arn:aws:ec2:*:*:instance/*","Condition":{"StringLike":{"ssm:resourceTag/CyberArkManaged":["true"]}}}`. The existing `SessionDocuments`+`ManageSessions` statements cover the rest.
- **Order:** the tag must be LIVE before the policy helps — **apply (not just merge) Part 1 first**. ⚠️ **Atlantis gotcha: approving the MR ≠ applying it.** A reviewer approval does NOT run `atlantis apply`; the tag stays absent until someone comments `atlantis apply` and it returns `Apply complete`. Verify `merged_at` + an Apply-complete note, never just the approval note.
- **E2E = the proof of Part 2** (which you can't read via sso-admin): as the operator profile, `aws ssm start-session --target <target> </dev/null` → `Starting session … sh-5.1$` (opens; `terminate-session` after) means both parts live; **negative control** to a non-tagged instance (e.g. an EKS node) MUST still `AccessDeniedException`. The before-fix denial on the same host is your baseline. Honest reporting: until this E2E passes, a colleague's "done" on the IdC half is UNVERIFIED — attribute it, don't assert it.

See also [[coext105281-cyberark-caa-uat]], [[coext105506-caa-uat-build-host]], [[eishelp-ad-group-requests]], skill `cyberark-eis-install` `eis-build-host-provision`.
