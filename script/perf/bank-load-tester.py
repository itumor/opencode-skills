#!/usr/bin/env python3
"""
Bank OpenLDAP Load Tester — runs locally on master or replica.
Uses ldapsearch/ldapadd/ldapmodify/ldapdelete via subprocess.
No SSH, no remote hosts. Designed for bank env (172.23.11.236/237).
"""
import subprocess
import time
import sys
...
BIN_PATH = "/opt/symas/bin:/opt/symas/sbin:" + os.environ.get("PATH", "/usr/bin")
...
"""
    ...
    def _run_op(self, op, **kwargs):
        ...
        try:
            ...
            rc = proc.returncode
            success = rc in (0, 32) if op in ('search', 'admin_search') else (rc == 0)
...
def main():
    parser = argparse.ArgumentParser(description='Bank LDAP Load Tester')
    parser.add_argument('--host', default='172.23.11.236', help='LDAP server host')
    parser.add_argument('--port', type=int, default=636, help='LDAP port (636 for LDAPS)')
    ...
    parser.add_argument('--no-tls', action='store_true', help='Use LDAP (not LDAPS)')
    args = parser.parse_args()

    tester = LDAPLoadTester(
        host=args.host, port=args.port, base_dn=args.base_dn,
        admin_dn=args.admin_dn, admin_pw=args.admin_pw,
        user_count=args.user_count, tls=not args.no_tls
    )

    print(f"=== Bank LDAP Load Test ===")
